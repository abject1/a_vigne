...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			...
			{label = 'Véhicule de Livraison',  value = 'mule2'},
			{label = 'Véhicule de Récolte',  value = 'tractor2'},
		}
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('a_vigne:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('a_vigne:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('a_vigne:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('a_vigne:putStockItems', itemName, count)
						end
...
AddEventHandler('a_vigne:hasEnteredMarker', function(zone)
	if zone == 'RaisinFarmBlanc' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect_blanc')
		CurrentActionData = {zone= zone}
	end

	if zone == 'RaisinFarmRouge' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect_rouge')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementVinBlanc' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_transform_vine_blanc')
		CurrentActionData = {zone= zone}
	end		

	if zone == 'TraitementVinRouge' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_transform_vine_rouge')
		CurrentActionData = {zone= zone}
	end	

	if zone == 'TraitementChampagne' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_transform_champagne')
		CurrentActionData = {zone= zone}
	end	
...
AddEventHandler('a_vigne:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		TriggerServerEvent('a_vigne:stopHarvest')
	end  
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		TriggerServerEvent('a_vigne:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('a_vigne:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('a_vigne:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('a_vigne:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('a_vigne:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('a_vigne:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('a_vigne:startSell', CurrentActionData.zone)
				end
